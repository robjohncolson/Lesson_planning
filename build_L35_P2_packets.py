...
# ── Low-level helpers (shared with Day 6/23 builders) ────────────────────
...
def add_callout(doc, title, body_lines, color_hex="CFE2F3"):
    """Single-cell 1x1 tinted box with bold title and bullet body."""
...
def bank_item_block(doc, q, *, label=None, space_after_inches=1.0):
    """Print a Savvas item with optional label, bolded prompt, and a student
    work-space of the given height."""
    if label:
        p = doc.add_paragraph()
        r = p.add_run(label)
        r.bold = True
...
def add_header_block(doc, *, for_teacher=False):
    ...
    doc.add_paragraph()
    # Framework header strip (evaluator-required fields)
