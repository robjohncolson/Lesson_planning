...
<script>
  ...
  // Lesson-sibling index (same `group` field) — fallback for nodes that lack
  // explicit prereq/rehearses/echoes edges so the panel always has somewhere
  // to navigate to.
  ...
  function renderDetail(node) {
    ...
    // "Other items in this lesson" fallback — always surface siblings so
    // teachers can walk a lesson even when explicit typed edges are missing.
    const siblings = (siblingsByLesson[node.group] || []).filter(id => id !== node.id);
    if (siblings.length) {
      ...
      siblingRow.innerHTML = '<strong>Other items in lesson ' + escHtml(node.group) +
        ':</strong> ' + siblings.map(id =>
          '<a class="neighbor sibling" data-target="' + escHtml(id) +
          '" style="border-left-color:#666;">' + escHtml(id) + '</a>'
        ).join("");
      detail.appendChild(siblingRow);
    } else if (!neighborHtml) {
