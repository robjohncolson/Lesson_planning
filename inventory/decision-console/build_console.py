...
FROZEN_HASHES = {
    ...
    REGISTRY_PATH: "a2fe278292e97a432a6275b807fdfe730ab67a38efd09a70e5241355959dba17",
    ...
    WAVE_PLAN_PATH: "26f578b7cba22397f4015509a17dd097cfd829a2f48250ebf779237d3b9ad82c",
    COURSE_MAP_PATH: "c8274dfbaab29424d0cb5f15093359bab948112dee70d432ede7597d0fd095c0",
    ...
    ALIAS_MAP_PATH: "27b8c13b6c2a03d1c33e86ce4007dcfa27a7e56717044b821ac7db934d2ede5e",
}
...
def main() -> None:
    ...
    guard(len(registry_rows) == 900, f"registry.jsonl must have exactly 900 lines, got {len(registry_rows)}")
    ...
    guard(
        len(retirement_gap_into_41) == 4,
        f"expected exactly 4 retirement-gap dropped_edges targeting 4-1-*, got {len(retirement_gap_into_41)}",
    )

    four_one_stranded = content_readiness["four_one_stranded"]
    readiness_edges = four_one_stranded["dangling_edges_into_4_1"]
    guard(len(readiness_edges) == 4, f"four_one_stranded.dangling_edges_into_4_1 must have 4 entries, got {len(readiness_edges)}")
    ...
    print("[guard] 4 dangling retirement-gap edges into 4-1 verified consistent across both sources")
    ...
    evidence = [
        ...
        {
            "fact": "0 registry rows exist for lesson 4-1 (grep-confirmed: grep -c '\"lesson\": \"4-1\"' questionbank/registry.jsonl -> 0).",
            "source": "inventory/topic-4-1/DIAGNOSIS.md line 92",
        },
        {
        ...
        {
            "fact": "Whether ingestion into the registry was briefly attempted and then culled during the skip, versus never begun past the skeleton-stub stage, is UNKNOWN from disk alone -- frame as \"never completed/retained, halted by the dept skip,\" not as a certain \"never ingested at all.\"",
            "source": "inventory/topic-4-1/DIAGNOSIS.md lines 132-158 (epistemic note)",
        },
    ]
    ...
    section1 = {
        ...
        "options": {
            "revive": {
                "downstream_effects": [
                    f"Unblocks the 4 retirement-gap edges into 4-1 ({edges_str})",
                ],
                "effort_paths": {
                    "revive-ingestion-only": (
                        "RECOVERY_PLAN Steps 2+4+5: hand-add 4 skeleton stubs for q20/q22/q25/q26, then "
                        "transcribe+append all 19 practice items SERIALLY via qb_append.py from the existing "
                        "screenshots, then backfill calibration anchors. NOT blocked on SE/TE."
                    ),
                    "revive-full-rebuild": (
                ...
                "risks": [
                    ...
                    "Ingestion effort (19 serial transcriptions) spent on a lesson that may never be taught",
                    "Full-rebuild path indefinitely blocked on SE/TE acquisition",
                ],
            },
            "retire": {
                "downstream_effects": [
                    f"The 4 lessons carrying dangling edges ({edges_str}) need bridge re-pointing",
                    "course_map.json regeneration to drop/re-point the edges",
                    "CLAUDE.md:45 stale 'ready' table needs correcting to retired",
                ],
                "effort": (
                    "4 bridge edge re-points across 4 lessons + course-map rebuild + CLAUDE.md correction; "
                    "no ingestion work"
                ),
                "risks": [
                    "Staged work (61 screenshots + authored calibration) permanently unused",
                    "Inverse variation on-ramp into Topic 4 rational functions lost from the graph",
                    "Re-pointed bridges may be pedagogically weaker than the original 4-1 targets",
                ],
            },
        },
        "recommendation": {
            ...
            "rationale": [
                "(a) 4 lessons carry dangling retirement-gap edges into 4-1 -- permanent retirement costs 4 bridge "
                "edits anyway",
            ...
            "residual_risk": (
                "If the 5/13 dept skip was department POLICY rather than time pressure, retire instead and "
                "re-point the 4 edges -- that is precisely the judgment only the teacher can make, hence 0.7 "
                "not higher."
            ),
        },
        "decision": {
    ...
    # rc-merge-auth-5-4-2026-07-23 (additive): dual-denominator identity,
    # computed from the wave plan actually loaded this run (real or fixture)
    # -- 'registry_rows' above stays the raw, audit-style total; this is the
    # authoritative place a console reader gets the RC-mandated active/alias
    # split. STUDENT-FACING selection (qb.py) is what actually resolves/
    # excludes merged-alias rows; this console only reports the count.
    merged_alias_item_uids_console = sorted(
        it["item_uid"] for it in all_wave_items if it.get("status") == "merged-alias"
    )
    active_registry_rows = len(registry_rows) - len(merged_alias_item_uids_console)
    guard(
    ...
    guard(
        active_registry_rows + len(merged_alias_item_uids_console) == len(registry_rows),
        "identity_reconciliation failed: active_registry_rows "
        f"({active_registry_rows}) + merged_alias_rows "
    ...
    meta = {
        ...
        "identity_reconciliation": {
            ...
            "statement": (
                ...
                "(unchanged by this merge); active_registry_rows is what a student-facing "
                "consumer (qb.py) actually selects from."
            ),
        },
