## Task: render the L41_P3 Teacher packet to LaTeX
...
1. `graph/eye_check/L41_P3.md` — the structured intent brief. The
...
4. `build_L41_P3_packets.py` — the Python builder for this lesson. Use
   only as a reference for visual hierarchy and bank-item ordering. Do
   NOT modify it.
5. `L41_P3_Teacher_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L41_P3_teacher.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L41_P3_teacher.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L41_P3_teacher.tex`. Do not modify the
