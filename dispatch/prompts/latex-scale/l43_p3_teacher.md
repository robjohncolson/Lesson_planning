## Task: render the L43_P3 Teacher packet to LaTeX
...
4. `build_L43_P3_packets.py` — the Python builder for this lesson. Use
   ...
   NOT modify it.
5. `L43_P3_Teacher_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L43_P3_teacher.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L43_P3_teacher.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L43_P3_teacher.tex`. Do not modify the
