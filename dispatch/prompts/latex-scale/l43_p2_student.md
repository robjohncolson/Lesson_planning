## Task: render the L43_P2 Student packet to LaTeX
...
1. `graph/eye_check/L43_P2.md` — the structured intent brief. The
...
4. `build_L43_P2_packets.py` — the Python builder for this lesson. Use
   only as a reference for visual hierarchy and bank-item ordering. Do
   NOT modify it.
5. `L43_P2_Student_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L43_P2_student.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L43_P2_student.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L43_P2_student.tex`. Do not modify the
