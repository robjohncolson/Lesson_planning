## Task: render the L54_P2 Teacher packet to LaTeX
...
1. `graph/eye_check/L54_P2.md` — the structured intent brief. The
...
4. `build_L54_P2_packets.py` — the Python builder for this lesson. Use
   only as a reference for visual hierarchy and bank-item ordering. Do
   NOT modify it.
5. `L54_P2_Teacher_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L54_P2_teacher.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L54_P2_teacher.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L54_P2_teacher.tex`. Do not modify the
