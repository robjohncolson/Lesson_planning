## Task: render the L54_P3 Student packet to LaTeX
...
4. `build_L54_P3_packets.py` — the Python builder for this lesson. Use
   ...
   NOT modify it.
5. `L54_P3_Student_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L54_P3_student.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L54_P3_student.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L54_P3_student.tex`. Do not modify the
