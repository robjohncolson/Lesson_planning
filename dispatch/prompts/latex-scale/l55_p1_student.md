## Task: render the L55_P1 Student packet to LaTeX
...
1. `graph/eye_check/L55_P1.md` — the structured intent brief. The
...
4. `build_L55_P1_packets.py` — the Python builder for this lesson. Use
   only as a reference for visual hierarchy and bank-item ordering. Do
   NOT modify it.
5. `L55_P1_Student_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L55_P1_student.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L55_P1_student.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L55_P1_student.tex`. Do not modify the
