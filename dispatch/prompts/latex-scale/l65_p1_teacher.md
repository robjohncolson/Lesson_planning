## Task: render the L65_P1 Teacher packet to LaTeX
...
4. `build_L65_P1_packets.py` — the Python builder for this lesson. Use
   ...
   NOT modify it.
5. `L65_P1_Teacher_Packet.docx` — canonical visual reference.
6. `questionbank/registry.jsonl` — read items by ID. Render `prompt` and
   `answers` (if any) verbatim — do NOT paraphrase.

### Output (one new file)

`tex/L65_P1_teacher.tex` — standalone document, starts with:
```
...
1. Run `pdflatex tex/L65_P1_teacher.tex --miktex-enable-installer`
...
- Write ONLY to `tex/L65_P1_teacher.tex`. Do not modify the
