...
\begin{callout}{\IconSpeak\ CHAIN 1 CALLBACK --- Storage Box (PLANT, April)}{calloutpeach}
\textit{When to say:} After students solve Practice \#27, before Share. This plants the year-long extract-hidden-variable archetype --- callbacks will land in L41, L51, L63, L64, L65.

\smallskip
``Notice what we just did: we had a physical object, we had one constraint (volume equals something), and we had to pull out a hidden dimension. That move --- \textbf{build a polynomial from a constraint, then extract a dimension} --- is a year-long move. We're going to see it again with a milk container in May, with a money formula in June. Today's work is how we learn the machine. Same move, new machine each time.''
\end{callout}
