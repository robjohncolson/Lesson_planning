...
\noindent\begin{tabular}{@{}>{\bfseries}p{1.6in}@{\hspace{8pt}}p{4.7in}@{}}
Math Objective & Construct a factored polynomial from a graph by identifying x-intercepts, choosing the correct multiplicity at each (touch = even, cross = odd), and applying end behavior to determine the leading coefficient sign. \\[4pt]
Language Objective & Students will use the frame ``The factor for $x = \framedline{0.6in}$ must be $\framedline{0.6in}$ because the graph $\framedline{0.9in}$'' to justify each factor of a polynomial, and explain why even multiplicity produces a touch while odd multiplicity produces a cross. \\[4pt]
Essential \mbox{Understanding} & A polynomial's graph and its factored form encode the same information --- zeros locate the factors, multiplicity determines the local behavior at each x-intercept (touch vs.\ cross), and end behavior determines the leading coefficient sign. \\
\end{tabular}
}
\end{tcolorbox}
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*]
    \item What is the multiplicity of the zero $x = 4$ in $f(x) = (x-4)^2(x+1)$? \framedline{2in}
...
\begin{tabular}{@{}>{\bfseries\raggedright}p{1.7in}@{\hspace{6pt}}>{\raggedright\arraybackslash}p{4.8in}@{}}
Zero $x = 2$ & multiplicity = \frameblank,\quad behavior at zero: \framedline{2.6in} \\[5pt]
Zero $x = -3$ & multiplicity = \frameblank,\quad behavior at zero: \framedline{2.6in} \\[5pt]
Degree & \frameblank,\quad leading coefficient sign: \framedline{2in} \\[5pt]
End behavior (left/right) & \framedline{2.2in},\quad \framedline{2.2in} \\[5pt]
\end{tabular}
...
\end{tikzpicture}
\end{center}
{\small\textit{Sketch the shape only --- exact y-values not required. Mark zeros and label touch/cross.}}

\textbf{Why?} \framedline{5.5in} \\[6pt]
\framedline{6in}
\end{samepage}
...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item End behavior: left \frameblank, right \frameblank \\
          \hspace*{1em}(Why? \framedline{4.5in})
\end{itemize}
