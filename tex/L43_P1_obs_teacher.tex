...
\begin{xltabular}{\linewidth}{|>{\bfseries\RaggedRight\arraybackslash}p{1.4in}|Y|>{\RaggedRight\arraybackslash}p{1.9in}|>{\itshape\RaggedRight\arraybackslash}p{1.55in}|}
  ...
  % ---------- DO NOW (NEW per Bonnevie meeting) ----------
  Do Now (10 min) \newline
  \emph{Function-family match + open/closed circle review, DOK 1--2}
  &
  Hand out student packet at the door. Project Page 1 (Do Now).     \newline
  \textbf{Part 1 (5 min):} Read aloud the prompt ``Match each graph A--D to its function family.'' Set timer 3 min silent matching, 1 min pair check, 1 min cold-call to share answers.     \newline
  Anticipated: A=quadratic, B=exponential, C=linear, D=rational.     \newline
  \textbf{Part 2 (4 min):} Read the two number lines. Ask ``what's different?'' Set 2 min silent + 2 min pair share. Surface: closed = included, open = excluded. Notation: Line 1 is $x \geq -3$; Line 2 is $x \neq 2$.     \newline\smallskip
  \textbf{Bridge to Explore \& Reason:} ``Today's family is \emph{rational} -- and the open circle on Line 2 shows up everywhere when we restrict domains.''
  &
  Look at each graph; recall what its parent function looks like.     \newline
  Match each to a word from the bank.     \newline
  Compare answers with elbow partner.     \newline
  Read both number lines; notice the open vs closed circle.     \newline
  Write a domain expression for each line.
  &
  How can you engage students in the lesson?     \newline
  How can you activate prior knowledge of function families?     \newline
  How are all adults intentionally planned for? \\ \hline

  % ---------- EXPLORE \& REASON (was Do Now in prior version) ----------
  Explore \& Reason (8 min) \newline
  \emph{$y=x+2$ with $x = 2$ excluded -- DOK 2 conceptual gateway}
  &
  Project the graph of $y = x+2$ from packet Page 2.     \newline
  Read aloud Parts A, B, C once. Set timer 5 min silent independent work.     \newline
  Then 2 min pair share. Cold-call 1--2 pairs to share Part C answer.     \newline\smallskip
  \textbf{Bridge to Example 6:} ``When the denominator equals zero, the function is undefined. That's a \emph{domain restriction} -- and we'll see SA/V ratios in the next two problems.''
  &
  Read the prompt silently.     \newline
  Answer Part A (domain of $y = x+2$).     \newline
  Sketch Part B (function with $x = 2$ excluded).     \newline
  Write Part C using the printed sentence frame.     \newline
  Volunteer (or be cold-called) to share.
  &
  How are you eliciting prior reasoning before introducing new content?     \newline
  How will you handle students who don't yet think of \emph{rational}? \\ \hline

  % ---------- EXAMPLE 6 (NEW guided bridge before Apply) ----------
  Example 6 (10 min) \newline
  \emph{Cube vs Cylinder SA/V (guided) -- DOK 2 procedural}
  &
  Project Page 3 (Example 6).     \newline
  \textbf{We do (4 min):} Walk through the \textbf{cube} side-by-side with students. Show $V = (2x)^3 = 8x^3$, $SA = 6(2x)^2 = 24x^2$, $SA/V = 3/x$.     \newline
  \textbf{You do (5 min):} Students compute the \textbf{cylinder} on their own using $V = \pi r^2 h$, $SA = 2\pi rh + 2\pi r^2$, with $r = x$, $h = 2x$. Circulate to verify they substitute correctly.     \newline
  \textbf{Comparison (1 min):} Cold-call. Both should be $3/x$ -- the surprise. Surface: ``equal ratios means choose by other criteria (cost, manufacturing).''
  &
  Take notes as teacher works the cube.     \newline
  Compute $V$, $SA$, and $SA/V$ for the cylinder independently.     \newline
  Compare with the cube ratio.     \newline
  Use the sentence frame to articulate the design conclusion.
  &
  What will you explicitly model and how?     \newline
  How will you check that students substitute $r = x$ and $h = 2x$ correctly?     \newline
  Are students ready for the harder Apply problem after this? \\ \hline

  % ---------- APPLY (now ramped down from 15->10 min since Example 6 set it up) ----------
  Apply (10 min) \newline
  ...
  % ---------- PERFORMANCE TASK (compressed 20->18 to fit new Do Now + Example 6) ----------
  Performance Task (18 min) \newline
  ...
  % ---------- SHARE / SUMMARY (compressed 5->4 to absorb new content) ----------
  Share / Summary (4 min) \newline
