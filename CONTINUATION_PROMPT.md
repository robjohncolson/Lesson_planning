...
- **sish-on-Railway tunnel bouncer** — *bundle written and committed to `connect` repo (`af1a61b`, 2026-05-03), awaiting Railway deploy.* Files in `connect/sish/` (Dockerfile, railway.json, README, pubkeys/) and `connect/athena/` (sish-reverse.sh, sish-reverse.service, install-sish.sh, README). Work-laptop client is `connect/connect-sish.bat`. Replaces the cloudflared dependency (Falcon quarantines `cloudflared.exe` / `tailscale.exe` on the work laptop). Existing cloudflared path stays in place as fallback. **Pending operator steps**: (1) `railway init` + `railway up` from `connect/sish/` and provision Railway volume at `/host-keys`; (2) set the documented sish env vars; (3) add 2 Railway TCP Proxy mappings (internal :2222 and :9000); (4) on Athena run `bash athena/install-sish.sh` (generates Athena's reverse-tunnel keypair, installs systemd unit), then commit the printed pubkey to `connect/sish/pubkeys/athena.pub` and `railway up` once more; (5) populate `connect/config.env` (Athena) and `config.bat` (work laptop) with `SISH_HOST`/`SISH_SSH_PORT`/`SISH_TCP_PORT`. See `connect/sish/README.md` for the full runbook.
