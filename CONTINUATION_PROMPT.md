...
- **Tunnel bouncer on Railway** — *bundle in `connect` repo (`5aa6a0d`, 2026-05-03), partial deploy.* Replaces the cloudflared dependency (Falcon quarantines `cloudflared.exe`/`tailscale.exe` on the work laptop). **Pivoted from sish → vanilla openssh-server after 3+ hours of sish quirks** (alias name validators rejecting "athena", "too many colons in address" config bugs, random per-session alias names, etc.). Final architecture: debian-slim + openssh-server with hardened sshd_config (`AllowUsers tunnel`, `PermitOpen localhost:9000`, pubkey-only). Files: `connect/sish/{Dockerfile,sshd_config,entrypoint.sh,README.md,railway.json,pubkeys/}` + `connect/athena/{sish-reverse.sh,sish-reverse.service,install-sish.sh,README.md}` + `connect/connect-sish.bat`. Existing cloudflared path stays in place as fallback. **Pending operator steps** (when picking back up): (1) Railway dashboard → service `sish` → **delete all SISH_* env vars** (the new image needs ZERO env vars); (2) push to main triggers GitHub auto-deploy; (3) on every client machine `ssh-keygen -R "[$SISH_HOST]:$SISH_SSH_PORT"` to clear the stale sish host-key entry from any prior tests; (4) on Athena `systemctl --user restart sish-reverse.service` and `journalctl --user -u sish-reverse.service -f` should show clean startup with no exit-code-255 loop; (5) test: `connect-sish.bat` from work laptop should land in Athena's WSL. Codex review of the openssh bundle attempted via cross-agent.py — timed out 3× this session in xhigh reasoning mode, matches CLAUDE.md gotcha #5. Self-review caught one tightening (`PermitOpen localhost:9000` vs `any`). See `connect/sish/README.md` for the operator runbook.
