...
- ✅ Registry answer backfill: `teacher_answer` field added to 88 registry rows, auto-extracted from `tex/L*_teacher.tex` via `extract_teacher_answers.py` (bankitem→teacheranswerbox pairing with fuzzy-prompt fallback). Generator `build_lesson_from_yaml.py:212-218` now emits backfilled body instead of placeholder. L41_P2 regen: 7/8 boxes filled; L44_P1 regen: 5/9 filled.
- ✅ Teacher-answer backfill round 2 (post-session, same day): extractor now handles `\teacheranswerbox` with 1-arg arity (L54_P1 pattern), normalizes LaTeX more aggressively (strips `\(`, `\dfrac`, punctuation), and fuzzy-matches via SequenceMatcher (ratio ≥ 0.80, margin ≥ 0.06 over runner-up) with in-file dedup. First-writer-wins policy on cross-tex collisions + never-overwrite-existing guard (preserves hand-polished `\textbf`/`\dfrac` formatting). Result: 88 → 128 backfilled rows (+40). L54_P1 0 → 6 pairs.
...
- **Teacher Console Phase 2:**
  ...
  - ✅ Curriculum DAG viz — reused existing `graph/graph.html` (vis-network, self-contained). Console adds DAG button that opens `/api/graph?lesson={group}` in a new tab; graph.html reads the param, fades non-focus nodes, fits camera to the lesson's subgraph, shows focus banner. Smoke: `smoke_test_console_dag.py`.
